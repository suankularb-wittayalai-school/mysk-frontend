...
const CheerHealthProblemLegend: StylableFC = ({ style, className }) => {
  ...
  return (
    <ul
      style={style}
      className={cn(
        `mx-4 flex flex-row flex-wrap justify-center gap-x-3 gap-y-1 rounded-md border-1 border-outline-variant p-2 pr-3 *:flex *:flex-row *:items-center *:gap-2 sm:mx-auto md:border-0 md:p-0 [&_div]:!w-5 [&_span]:text-on-surface-variant`,
        className,
      )}
    >
      <li>
